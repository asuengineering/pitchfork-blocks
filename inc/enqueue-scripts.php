...
function pitchfork_blocks_enqueue_block_styles() {
		...
		$plugin_version     = $the_version . '.' . filemtime( plugin_dir_path( __DIR__ ) . 'css/blocks.min.css' );
		$hero_video_version = $the_version . '.' . filemtime( plugin_dir_path( __DIR__ ) . 'js/hero-video.js' );

		wp_enqueue_style( 'pitchfork-block-styles', plugin_dir_url( __DIR__ ) . 'css/blocks.min.css', array( 'pitchfork-styles' ), $plugin_version );
...
function pitchfork_blocks_enqueue_block_scripts() {
	...
	$block_styles_version     = $the_version . '.' . filemtime( plugin_dir_path( __DIR__ ) . 'css/blocks.min.css' );
	$hero_video_version       = $the_version . '.' . filemtime( plugin_dir_path( __DIR__ ) . 'js/hero-video.js' );
	...
	wp_enqueue_script( 'hero-video-controls', plugin_dir_url( __DIR__ ) . 'js/hero-video.js', array( 'jquery' ), $hero_video_version, true );
	wp_enqueue_style( 'pitchfork-block-styles', plugin_dir_url( __DIR__ ) . 'css/blocks.min.css', array(), $block_styles_version );
...
function pitchfork_blocks_enqueue_admin_scripts() {
	...
	$the_plugin     = get_plugin_data( plugin_dir_path( __DIR__ ) . 'pitchfork-blocks.php' );
	$the_version    = $the_plugin['Version'];
	$plugin_version = $the_version . '.' . filemtime( plugin_dir_path( __DIR__ ) . 'css/block-editor.min.css' );

	wp_enqueue_style( 'pitchfork-block-admin-styles', plugin_dir_url( __DIR__ ) . 'css/block-editor.min.css', array(), $plugin_version );
